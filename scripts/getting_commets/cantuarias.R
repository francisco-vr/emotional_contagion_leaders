...
## Obtener comentarios e información de videos de cantuarias

cantuarias_list <- c("e4psjnu-O8k", "e_39GA7EbSA", "EsHnYR52iMU", "MoP9n5w98TU", "Admpb5RxaD0", "iggcwCX_kqE", "fwmyw7t7ZWo", "QDjmvN0iAgo",
                     "C6M3LMktSkc", "9S21RWsRN64", "tEQwzp4tUMg", "oPnnh32BqpU", "mVxUQzpixdM", "KZWaFvmHmYs", "8Fg44VMrNBQ", "XBAhTP6fJhY",
                     "Au9oB9NMdMA", "bLRu9XivQFQ", "Jvrm_TyjT-g", "SHGiKAg-BxQ")

## Mucho ojo: están usando imagenes en videos

comments_df <- data.frame() # data frame vacío donde se almacenarán los comentarios

for (video_id in cantuarias_list){
...
write.csv(comments_df, file = "Data/InputData/youtube_df/comments/comentarios_cantuarias.csv")
...
write.csv(video_details, file = "Data/InputData/youtube_df/video_details/video_details_cantuarias.csv")


#unimos comentarios con detalles del video


cantuarias_df <-merge(video_details,comments_df, by.x ="id", by.y="videoId")


cantuarias_df <-cantuarias_df%>%
  dplyr::select(id, title, date, views,likes,comment_count,textOriginal, authorDisplayName, likeCount, publishedAt)

write.csv(cantuarias_df, file = "Data/InputData/youtube_df/merged_df/cantuarias_final.csv")
