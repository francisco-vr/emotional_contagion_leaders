...
## Obtener comentarios e información de videos de mochiatti

mochiatti_list <- c("pilmy3WBDpY", "RSTL6rpjF-g","UfF8DwLCjlg", "v-ZLcMXpBkA", "AI_VS6Cj9oo", "5kHcGQhwh40", "uL4kyVa6bdU", "Sdryz-cO9Bk", "20iTCG7iBUA",
                ...
                "s2NIGPa2v7U", "I9BxL9CPNnI", "v3F0kq1DAOo", "29WsrvmYOro", "HL2Qvzw9Jyw","JSdrtSElN-8", "44J-78RvHwM")

comments_df <- data.frame() # data frame vacío donde se almacenarán los comentarios

for (video_id in mochiatti_list){
...
write.csv(comments_df, file = "Data/InputData/youtube_df/comments/comentarios_mochiatti.csv")
...
mochiatti_df <-merge(video_details,comments_df, by.x ="id", by.y="videoId")


mochiatti_df <-mochiatti_df%>%
  dplyr::select(id, title, date, views,likes,comment_count,textOriginal, authorDisplayName, likeCount, publishedAt)

write.csv(mochiatti_df, file = "Data/InputData/youtube_df/merged_df/mochiatti_final.csv")
